...
class KnpBase:
    ...
    @classmethod
    def __contains_toolong_character(
        cls,
        s: str,
        character_pattern: str
    ) -> bool:
        rs = re.findall(character_pattern, s)
        return rs and max([len(r) for r in rs]) > cls.MAX_CONSECUTIVENESS

    def contains_toolong_alphabet(
        self,
        s: str,
    ) -> bool:
        return self.__contains_toolong_character(s, r'[^\p{Hiragana}\p{Katakana}\p{Han}\p{N}]+')

    def contains_toolong_number(
        self,
        s: str
    ) -> bool:
        return self.__contains_toolong_character(s, r'[\p{N}.．]+')

    def contains_toolong_han(
        self,
        s: str
    ) -> bool:
        return self.__contains_toolong_character(s, r'[\p{Han}]+')

    def contains_toolong_kana(
        self,
        s: str
    ) -> bool:
        return self.__contains_toolong_character(s, r'[\p{Katakana}ー]+')

    def contains_toolong_hira(
        self,
        s: str
    ) -> bool:
    ...
    def __juman_lines(
            self,
            text: str,
            jumanpp: Optional[bool] = None
    ) -> str:

        if len(text.encode('utf8')) > self.MAX_JUMAN_BYTELENGTH:
            raise InputTooLongError(f'input size is {len(text.encode("utf8"))} > {self.MAX_JUMAN_BYTELENGTH}')
    ...
    def __knp_parse(
        ...
        if self.knp.socket:
            knp_lines = self.knp.socket.query(juman_str, pattern=r'^%s$' % self.knp.pattern)
        else:
            knp_lines = self.knp.subprocess.query(juman_str, pattern=r'^%s$' % self.knp.pattern)
    ...
    def unit_midasi(
        ...
        if ignore_genkei is None:
            ignore_genkei = set()
        mrphs = [m for m in unit.mrph_list()
    ...
    def unit_genkei(
        ...
        return cls.unit_midasi(unit, ignore_pos=ignore_pos, ignore_genkei=ignore_genkei, genkei=True)
    ...
    def extract_negation(
        ...
        """
        if ('準否定表現' in tag.features or '否定表現' in tag.features) and '二重否定' not in tag.features:
            return True
        return False
